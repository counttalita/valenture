...
<div class="container mx-auto px-4 py-6" x-data="learnerProgress()" x-init="init()">
    ...
    <div class="mb-8 flex flex-col sm:flex-row items-center gap-4">
        ...
        <div class="w-full sm:w-auto bg-white rounded-xl shadow-md px-4 py-3 flex items-center gap-3 relative">
            <div class="relative w-full">
                <select x-model="sortOrder" @change="sortLearners()" class="appearance-none w-full pl-10 pr-8 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 bg-white text-gray-800 text-base shadow-sm transition-all duration-200">
                    <option value="default">Sort: Default</option>
                    <option value="desc">Sort: Highest Progress</option>
                    <option value="asc">Sort: Lowest Progress</option>
                </select>
                <div class="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none">
                    <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
                </div>
    ...
    (learner.lastname && learner.lastname.length > 0 ? learner.lastname[0] : '?')).toUpperCase()"></span>
            ...
            <div>
                <h2 class="font-semibold text-lg text-gray-900 flex items-center gap-2">
                    <span x-text="(learner.firstname ?? '') + ' ' + (learner.lastname ?? '')"></span>
                    <span class="ml-2 text-sm font-semibold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded" x-text="'Overall: ' + (getOverallProgress(learner)) + '%' "></span>
                </h2>
        ...
        <div class="ml-2 mt-2">
            ...
            <template x-for="sortedCourse in getSortedCourses(learner)" :key="sortedCourse.id">
                <div class="mb-2">
                    <div class="flex justify-between items-center">
                        <span class="text-gray-800 font-medium" x-text="sortedCourse.name"></span>
                        <span class="text-xs text-gray-500 font-mono" x-text="(sortedCourse.pivot.progress ?? 0) + '%' "></span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2.5 mt-1">
                        <div class="bg-gradient-to-r from-blue-400 to-indigo-500 h-2.5 rounded-full transition-all duration-300" :style="'width: ' + (sortedCourse.pivot.progress ?? 0) + '%'" role="progressbar" :aria-valuenow="sortedCourse.pivot.progress ?? 0" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                </div>
            </template>
        </div>
...
function learnerProgress() {
    return {
        ...
        sortLearners() {
            if (this.sortOrder === 'default') return;
            this.learners.sort((a, b) => {
                const aProgress = this.getOverallProgress(a);
                const bProgress = this.getOverallProgress(b);
                if (this.sortOrder === 'asc') return aProgress - bProgress;
                if (this.sortOrder === 'desc') return bProgress - aProgress;
                return 0;
            });
        },
        getOverallProgress(learner) {
            if (Array.isArray(learner.courses) && learner.courses.length > 0) {
                let total = 0, count = 0;
                learner.courses.forEach(c => {
                    if (c.pivot && typeof c.pivot.progress === 'number') {
                        total += c.pivot.progress;
                        count++;
                    }
                });
        ...
        getSortedCourses(learner) {
            if (!Array.isArray(learner.courses)) return [];
            let courses = [...learner.courses];
            if (this.sortOrder === 'default') return courses;
            return courses.sort((a, b) => {
                const aP = a.pivot && typeof a.pivot.progress === 'number' ? a.pivot.progress : 0;
                const bP = b.pivot && typeof b.pivot.progress === 'number' ? b.pivot.progress : 0;
                if (this.sortOrder === 'asc') return aP - bP;
                if (this.sortOrder === 'desc') return bP - aP;
                return 0;
            });
        },
        applyCourseFilter() {
